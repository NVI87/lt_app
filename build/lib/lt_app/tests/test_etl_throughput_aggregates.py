...
def _build_monitor_fixture_csv(tmp_path: Path) -> Path:
    r"""
    Создать CSV-фикстуру мониторинга с двумя индексами за 4 минуты.
    ...
    - Минута 10:01: НЕТ валидных строк ни для одного индекса — полностью
      пропущенная минута. reindex+ffill должны добавить её с carried-forward
      значениями, delta=0.
    - Минута 10:02: индекс "a" 2 опроса (last=55), индекс "b" только error-
      строки (проверка forward-fill: не должно быть false drop).
    - Минута 10:03: оба индекса с валидными опросами (a last=75, b last=50).
    """
    records: list[dict] = []
    ...
    # Minute 10:01 — fully missing (no valid rows for either index)

    # Minute 10:02 — "b" has only error rows
    records.append({
        "timestamp": "2025-06-01T10:02:15Z",
        "index_name": "a",
        "document_count": 45,
        "error": None,
    })
    records.append({
        "timestamp": "2025-06-01T10:02:45Z",
        "index_name": "a",
        "document_count": 55,
        "error": None,
    })
    ...
    # Minute 10:03 — both valid
    records.append({
        "timestamp": "2025-06-01T10:03:10Z",
        "index_name": "a",
        "document_count": 65,
        "error": None,
    })
    records.append({
        "timestamp": "2025-06-01T10:03:50Z",
        "index_name": "a",
        "document_count": 75,
        "error": None,
    })
...
async def test_builder_happy_path(tmp_path: Path):
    """Полный проход: 4 минуты (20 строк), reindex заполняет пропущенную минуту,
    forward-fill на error-минуте, корректные дельты."""
    ...
    assert result.minute_buckets == 4  # includes reindexed missing minute 10:01
    ...
    # Cumulative totals after ffill+reindex:
    # min 0: a=35 b=30=65, min 1(ffill): a=35 b=30=65,
    # min 2: a=55 b(ffill)=30=85, min 3: a=75 b=50=125
    assert row["total_indexed_documents"] == 125
    assert row["total_sent_messages"] == 100
    assert row["coverage_ratio"] == pytest.approx(1.25)

    # Deltas: min1 65-65=0, min2 85-65=20, min3 125-85=40
    # The missing-minute delta is 0 (reindex+ffill), not a multi-minute span
    assert row["avg_per_minute"] == pytest.approx(20.0)
    assert row["max_per_minute"] == pytest.approx(40.0)

    # 5-min trailing mean: [0, (0+20)/2=10, (0+20+40)/3=20]
    assert row["avg_per_minute_smoothed_5min"] == pytest.approx(10.0)
    assert row["max_per_minute_smoothed_5min"] == pytest.approx(20.0)
